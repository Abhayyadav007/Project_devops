...
export default function OverviewCards({ role }: OverviewCardsProps) {
  const [stats, setStats] = useState<{ label: string; value: number; change: string }[]>([]);
  ...
  useEffect(() => {
    async function fetchStats() {
      setLoading(true);
      try {
        const data = await getOverviewStats(role);
        setStats(data);
      } catch {
        setStats([]);
      } finally {
  ...
  return (
    <div className="animate-fade-in">
      ...
      {loading ? (
        <div style={{ color: "var(--text-muted)", fontSize: "var(--font-size-sm)", padding: 20 }}>
          Loading stats...
        </div>
      ) : stats.length === 0 ? (
        <div style={{ color: "var(--text-muted)", fontSize: "var(--font-size-sm)", padding: 20 }}>
